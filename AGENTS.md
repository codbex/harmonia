...
**Keep this file in sync with [CONTRIBUTING.md](CONTRIBUTING.md).** That file is the human-facing counterpart of this one, covering the same conventions in plain language for people. When you change a convention here, make the matching change there in the same commit, and vice versa. The only exception is an instruction that exists purely to constrain a coding agent (the em dash rule below, for example), which stays here alone.
...
- **Use `rem`, not `px`** in styles, including inline styles set from JS, unless rem isn't possible or pixels are a better fit. Convert measured pixel values (`getBoundingClientRect`) to rem.
...
- **Prefer `aria-*` over a `data-*` twin.** When a state can be expressed with an `aria-*` attribute (`aria-pressed`, `aria-expanded`, `aria-selected`, `aria-checked`, `aria-disabled`, ...), use that attribute and style from it. Do not add a `data-*` attribute for the same state, because the two then have to be kept in sync by the consumer or by an observer inside the library.
...
- **Every doc page follows one canonical structure, enforced by `tests/docs-structure.test.js`** (the source of truth for doc layout): `# Title` -> one-paragraph description -> `## Usage` -> optional `## Keyboard Handling` / `## Accessibility` -> `## API Reference` -> `## Examples`. Under API Reference: `### Component attribute(s)` (always, plural) then optional `### Attributes` (per-directive `#### x-h-<name>` sub-tables), `### Modifiers` (after Attributes), `### Model`, `### Events`, `### Configuration` (never `Config`), `### CSS Variables` (`| Variable | Default | Description |`, after Configuration) - for a component-scoped CSS custom property that ships a default but is deliberately not a global theme token (e.g. `--sidebar-width`, `--bg-bubble`); such a variable must never be added to `src/styles/globals.css` or to the theme generator's `colorVars`/`shadowVars`/`fontVars`/`othersVars` buckets, which are reserved for actual theme tokens declared on `:root`/`.dark`. Example variants are `###` subsections under a single `## Examples`. The test also checks section order and canonical names; run `npx vitest run tests/docs-structure.test.js` after editing docs.
...
- **Agent docs are generated from `docs/**`, not hand-written.** `scripts/generate-agent-docs.cjs`transcribes each doc into`skills/harmonia/`(the`SKILL.md`router, one`references/<name>.md`per doc, and`llms.txt`), which ships in the npm package and doubles as a Claude Code plugin (`.claude-plugin/`). It runs on every `npm run build`; `tests/agent-docs.test.js`fails if`skills/`is stale or a component lacks a reference. So the source docs are the single source of truth: to change agent-facing content, edit the doc (or the generator's parser/template) and rerun`npm run agent-docs:generate`. Do not hand-edit files under `skills/`. The generator relies on the doc template (an `## API Reference`section with a`Component attribute(s)`block and ```html example blocks); a doc that drops the directive block trips a build warning and the test. The one reference NOT built from`docs/\*\*`is`references/utility-classes.md`: it is generated from `src/styles/harmonia.css`(the Tailwind`@source inline`safelist plus`@utility`/`.class`rules), which is the authoritative list of shipped utility classes - so adding or removing a utility class there automatically updates the agent allowlist. Likewise, `references/migration.md`is generated from the root`CHANGELOG.md`'s `- **Breaking: ...**`bullets, not from`docs/\*\*`- breaking changes only, by version, so a coding agent migrating between Harmonia versions gets signal without wading through every feature and fix. A future breaking change must use that exact bullet prefix or it will not appear in the generated reference; the generator warns (failing`tests/agent-docs.test.js`) when a version's intro paragraph mentions "breaking" with no matching bullet, but that is a safety net, not a substitute for using the convention. To keep a piece of docs prose (e.g. a photo-attribution note) visible on the docs site but out of the generated skill, wrap it in a`<!-- skill:ignore -->`...`<!-- /skill:ignore -->`region; the generator drops the whole region (the prose counterpart of`data-exclude="skill"`on a`<LiveExample>`).

## Versioning

Strict semantic versioning, no exceptions. **MAJOR** is the only release that may break existing usage. **MINOR** adds components and features and never breaks. **PATCH** is fixes only and never breaks. A breaking change therefore forces a major bump. There is no backwards-incompatible change small enough to ride along in a minor, and an earlier deviation permitting exactly that has been withdrawn. `tests/versioning.test.js` enforces this against `CHANGELOG.md` from v3.1.1 onward (the releases before it predate the rule). If a change cannot be made backwards compatible, it waits for the next major.

Every breaking change is recorded in `CHANGELOG.md` with the `- **Breaking: ...**` bullet prefix (see the Documentation section) and named in the release's intro paragraph. A version containing `alpha`, `beta` or `rc` publishes to npm under the `next` tag instead of `latest`.

### Support policy

The current major receives everything. The major before it receives guaranteed security fixes, and bug-fix backports only where the change is small enough to be worth it. It stays supported for as long as it remains the previous major, and for at least 12 months after its successor was released. Anything older receives nothing. The policy is published in `SECURITY.md` and `docs/versioning-and-support.md`, and those two plus `CONTRIBUTING.md` must agree.
