...
::: info
This plugin is **not** part of the default Harmonia bundle. You opt in by loading it, and i18next itself must be available as a global (`window.i18next`) before Alpine starts.
:::
...
Renders the translation into the element's text content (always as plain text, never as HTML). The translation key comes from the expression. The element's initial text content is a fallback, shown while the key cannot be resolved (i18next not yet initialized, or the key missing from the loaded resources). Do not combine it with `x-text` on the same element as both write the text content.
...
The plugin adds an `i18next` object to `Harmonia.plugins` (the ESM build exports the same functions by name) for configuring the [language synchronization](#language-synchronization-across-tabs-and-iframes):
