...
Use the Date Time Picker when a single value needs both a calendar date and a time of day (e.g. scheduling an appointment). For a date only, use the [Date Picker](/components/date-picker). For a time only, use the [Time Picker](/components/time-picker).
...
The calendar is exposed as an ARIA date grid (see the [Inline Calendar](/components/calendar) accessibility notes). Its month and year are toggle buttons that open a month grid or a year list in place of the days, as described in the [Inline Calendar](/components/inline-calendar#behavior) behavior. Each time segment is a `spinbutton` with its own label and `aria-valuemin` / `aria-valuemax` / `aria-valuenow` / `aria-valuetext`, so assistive technology announces it and its current value. The trigger advertises `aria-haspopup="dialog"` and its expanded state.
