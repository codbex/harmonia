...
A horizontal navigation bar where items are either direct links or triggers that open a dropdown [Menu](/components/menu). Intended for top-level site or application navigation.
...
`x-h-nav-trigger` acts as a menu trigger for the `x-h-menu` that follows it. The `x-h-menu` must be placed somewhere after the `x-h-nav-trigger` and they must share the same direct parent (`x-h-nav-item`). Do not add `x-h-menu-trigger` as the nav trigger sets up the trigger interface itself.
:::
