...
For a compact calendar that selects a single date or a date range, see [Inline Calendar](/components/inline-calendar).
...
| draggable        | Enable drag-and-drop rescheduling of events in the month, week, and day views. Defaults to `false`. See [Behavior](#behavior).                                                                    |
