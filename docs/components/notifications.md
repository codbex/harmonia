...
The `native` object bridges to the browser's Notification API for notifications shown by the operating system. It is fully separate from the in-page notifications above, so you decide when a message appears inside the page and when it goes to the system. See the [Native Notifications](/utilities/native-notifications) utility for details, including image support.
...
<LiveExample data-class="p-0 border-0">
