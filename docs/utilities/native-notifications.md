...
These helpers are fully independent from the [Notifications](/components/notifications) component, so you decide when a message should appear inside the page and when it should go to the operating system. They are also available inside Alpine markup through the `$notifications.native` magic object.
