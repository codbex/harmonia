...
When `tooltip` is enabled, hovering and clicking slices emit bubbling `CustomEvent`s on the chart element. See [the events reference](/charts/pie#events) for the shared `detail` shape.
